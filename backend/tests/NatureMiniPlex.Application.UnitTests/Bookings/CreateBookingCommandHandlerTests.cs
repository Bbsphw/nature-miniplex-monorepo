...
public class CreateBookingCommandHandlerTests
{
    ...
    {
        // Arrange
        var showtime = new Showtime { Id = 1, ShowDateTime = DateTime.UtcNow.AddHours(2) };
        ...
        var command = new CreateBookingCommand(1, "0812345678", null, new List<int> { 1 });

        // Act & Assert
        var ex = await Assert.ThrowsAnyAsync<Exception>(() => handler.Handle(command, CancellationToken.None));
    ...
    {
        // Arrange
        var showtime = new Showtime { Id = 1, TicketPrice = 100, ShowDateTime = DateTime.UtcNow.AddHours(2) };
        _mockShowtimeRepository.Setup(x => x.GetByIdAsync(1, It.IsAny<CancellationToken>())).ReturnsAsync(showtime);
        ...
        var command = new CreateBookingCommand(1, "0812345678", null, new List<int> { 1, 2 });
