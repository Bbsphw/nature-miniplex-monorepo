...
public interface IShowtimeRepository : IRepository<Showtime>
{
    ...
    Task<IReadOnlyList<NatureMiniPlex.Core.Application.DTOs.ShowtimeDto>> GetPagedShowtimesAsync(int? movieId, int? cinemaId, DateTime? date, int pageNumber, int pageSize, CancellationToken cancellationToken = default);
}
