...
public record GetShowtimesQuery(int? MovieId, int? CinemaId, DateTime? Date, int PageNumber = 1, int PageSize = 20) : IRequest<IReadOnlyList<NatureMiniPlex.Core.Application.DTOs.ShowtimeDto>>;
...
public class GetShowtimesQueryHandler : IRequestHandler<GetShowtimesQuery, IReadOnlyList<NatureMiniPlex.Core.Application.DTOs.ShowtimeDto>>
{
    ...
    {
        var cacheKey = $"Showtimes_{request.MovieId}_{request.CinemaId}_{request.Date:yyyyMMdd}_{request.PageNumber}_{request.PageSize}";
        ...
        if (!_cache.TryGetValue(cacheKey, out IReadOnlyList<NatureMiniPlex.Core.Application.DTOs.ShowtimeDto>? showtimes))
        {
            showtimes = await _showtimeRepository.GetPagedShowtimesAsync(
                request.MovieId, request.CinemaId, request.Date, request.PageNumber, request.PageSize, cancellationToken);
